...
for i in range(0, np.size(pos)):
    load_sin[i] = (sim_amp*np.sin((np.pi/sim_phase) + (2*np.pi/pos_num)*pos[i]) - sim_offset + np.random.randint(band_low, band_high) +
...
load_cal = calibration_wave(wave = load_wave, width = w_, sigma = sig_)

#generate filtered wave from calibration wave
filtered_wave = adjust_load(wave = load_cal, calibration_wave = cal_wave, width = w_, sigma = sig_)

#generate moving average from filtered wave
#smooth_wave = moving_avg(filtered_wave, win_len=wl_)

plt.figure(4)
plt.plot(load_cal[:, 0], load_cal[:, 1], 'r1')
#plt.plot(torque_wave[:, 0], torque_wave[:, 1], 'y1')
#plt.plot(smooth_wave[:, 0], smooth_wave[:, 1], 'g1')

cal_wave_ = np.column_stack((cal_wave[:,0], cal_wave[:,1]))

#conv_wave = signal.deconvolve(load_cal[:,1], cal_wave[:,1])
conv_wave = signal.deconvolve(cal_wave_[:,1], load_cal[:,1])

plt.figure(5)
plt.plot(conv_wave[:1])